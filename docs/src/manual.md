...
Because the struct is nothing but the coefficient,
`Vector{Decimal{P,S,T}}` has the layout of an Arrow/Parquet/DuckDB decimal
buffer with the same precision and scale, so data crossing that boundary
needs no per-element conversion.
...
maximum: `Decimal{7,2}` accepts fewer digits than `Decimal32{2}` before
...
[`DecimalValue{T}`](@ref DecimalValue) is the runtime-scale sibling: it
...
until run time, or the result of [`Decimals.normalize`](@ref). It is still
...
Both are subtypes of `Decimals.AbstractDecimal <: Real`. There is no
...
float conversions in Base do; a `DecimalValue` built from a float instead keeps
...
Everything else, including `convert` and the integer and rational
constructors, raises `InexactError` or `OverflowError` rather than losing a
digit.
...
[`rescale`](@ref); reach for `rescale` when the intent is "round this into
that scale".
...
target scale. The string constructors (`Decimal64{2}("1.25")`) and `dec"..."`
...
[`writedecimal!`](@ref Decimals.writedecimal!) is the allocation-free
...
[`Decimals.normalize`](@ref) is the other representation-level tool: it
strips trailing zeros from the coefficient and lowers the scale to match,
returning a `DecimalValue`, since the scale is then a property of the value
...
!!! note
    ...
    `LinearAlgebra.normalize`; call it as `Decimals.normalize`.
...
types, so plain `parse(Decimal64{2}, s)` is the same code path once Parsers
is loaded.
...
lost. `%d` is exact-or-`InexactError`, matching `Rational`.
...
Decimals serialize as JSON numbers carrying their exact digits, neither as
